...
def start_crawling():
    ...
    try:
        ...
        seeds_string = request.form["seeds"]
        seeds = seeds_string.split(" ")
        # if len(seeds) == 0:
        #     raise Exception()
        time_string = request.form["total_time"]
        print("Time:" + time_string)
        total_seconds = (int(time_string[0:2])
                         * 3600) + (int(time_string[3:5]) * 60)
        # go_to_stat = Timer(2.0, go_to_crawler_stat)
...
def calculate_user_bio_keywords():
    ...
    if request.method == 'POST':
        try:
            ...
            if request.form.get("stopwords"):
                filterStopwords = True
            keywords = user_analyzer.get_user_bio_keywords(
                user_bio, filterStopwords)
            return render_template('test.html', bioKeywords=keywords)
        except:
...
@app.route('/crawler/stats/deleted_users/', methods=['GET'])
def view_deleted_users():
    formatted_deleted_users = []
    for del_u in crawler.deleted_users:
        formatted_deleted_users = {
            "user": del_u[0],
            "exception": del_u[1],
        }
    return render_template('crawler_deleted_users.html', crawler=crawler, deletedUsers=formatted_deleted_users)
